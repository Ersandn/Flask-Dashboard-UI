...
def index():
    ...
    stats = {
        ...
        'rev_table' : playstore.groupby(['Category','App']).sum()[['Reviews','Rating']].
    ...
    columns='Total',
    values=df2['Installs'],
    aggfunc='sum').sort_values(by='Total', ascending=False).reset_index().head().set_index('Genres')
    #pie.plot.pie(y='Total')
